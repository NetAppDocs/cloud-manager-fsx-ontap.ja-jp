...
+
image:screenshot-fsx-file-system-id.png["AWS 管理コンソールの FSX ファイルシステム ID のスクリーンショット。"]

. [ ネットワークとセキュリティ *] タブで、優先サブネットまたはスタンバイサブネットのネットワークインターフェイス ID をクリックします。
+
image:screenshot-fsx-eni-id.png["ONTAP ENI ID の FSX のスクリーンショット。"]

. ネットワーク・インターフェイス・テーブルのセキュリティ・グループまたはネットワーク・インターフェイスの * 詳細 * セクションをクリックします。
+
image:screenshot-fsx-security-group-id.png["ONTAP セキュリティグループ ID の FSX のスクリーンショット。"]
